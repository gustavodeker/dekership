# Modelagem de banco (MariaDB VPS)

## Tabelas existentes
- `usuario` (manter).
- `ranking` (legado, pode coexistir durante migracao).
...
## Operacao atomica no fim da partida
- `START TRANSACTION`.
- Atualizar `game_match` com vencedor/perdedor.
- `INSERT ... ON DUPLICATE KEY UPDATE` em `player_stats` para 2 jogadores.
- `COMMIT`.

## Queries principais
```sql
SELECT u.usuario, s.wins, s.losses
FROM player_stats s
